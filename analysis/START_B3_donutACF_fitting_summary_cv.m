function START_B3_donutACF_fitting_summary_cv(varargin)
% summarizes cross-validated donut ACF results using saturation dots and
% testing 95% of the null distribution against 0
% 
...
for subjectI = 1:numel(subjectStrs)
    MAT_donutACF                            = fullfile(projectPath, 'results', 'moranI', 'START_B3_donutACF_cv', sprintf('START_B3_donutACF_cv_%s_%s_%s.mat', subjectStrs{subjectI}, taskStr, signalType)); 
...
for subjectI=1:numel(subjectStrs)
    ...
    for arrayI=1:numel(arrayStrs)
        ...
        yticks(minMoranI:0.2:maxMoranI);
        yticklabels(minMoranI:0.2:maxMoranI);
        xticks(0:0.4:3.7);
        xticklabels(0:0.4:3.7); 
        xlabel('distance (mm)');
        ylabel('correlation'); 
...
close all; 
end % function START_B3_donutACF_fitting_summary_cv
