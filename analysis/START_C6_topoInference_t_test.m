function START_C6_topoInference_t_test
% time series, ignoring events 
% the replicability of correlation matrices within and across tasks. 
% 
...
MAT_output_simmats              = fullfile(resultsPath, '..', 'START_C6_topoInference_generation', sprintf('output_simmats.mat')); 
...
PS_corr_summary                 = fullfile(resultsPath, sprintf('%s_corr_summary.pdf', currfilename));
...
for subjectI = 1:numel(subjectStrs)
    ...
    for arrayI = 1:numel(arrayStrs)        
        simmat_corrs        = nan(n_sessionPairs_thisSubject, 1);
        simmat_corrs_perm   = nan(n_sessionPairs_thisSubject, nPermutations); 
        ...
        for sess_pairI = 1:n_sessionPairs_thisSubject
            ...
            simmat1     = output_simmats.(task1).(subjectStrs{subjectI}).(arrayStrs{arrayI}).(sprintf('sess_%s', sess1)).signal_corrmat;
            
            simmat2     = output_simmats.(task2).(subjectStrs{subjectI}).(arrayStrs{arrayI}).(sprintf('sess_%s', sess2)).signal_corrmat;               
            chanLinearInds2     = output_simmats.(task2).(subjectStrs{subjectI}).(arrayStrs{arrayI}).(sprintf('sess_%s', sess2)).chanLinearInds;
            
            simmat_corrs_perm(sess_pairI, :) = corrmat_corrs_permutation(simmat1, simmat2, chanLinearInds2);

            simmat1     = sqmat2vec(simmat1); 
            simmat2     = sqmat2vec(simmat2); 

            simmat_corrs(sess_pairI) = corr(simmat1, simmat2, 'type', 'Pearson', 'rows', 'complete');
        end % sess_pairI
        ...
        actual_mean_task_combs  = nan(n_task_combinations, 1);
        perm_results_task_combs = nan(n_task_combinations, nPermutations);
        ...
        for combI=1:n_task_combinations
            ...
            actual_mean_task_combs(combI) = mean(actual_r_this_comb); 
            errorbar(1, actual_mean_task_combs(combI), std(actual_r_this_comb)/sqrt(n_task_pairs_combI)', '.', 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1);
            xlim([0, 1+1]);
            ...
            perm_results        = squeeze(mean(simmat_corrs_perm(inds_combI, :), 1)); % 1 x 1000
            perm_mean           = mean(perm_results); 
            perm_ste            = std(perm_results);
            perm_results_task_combs(combI, :) = perm_results; 
            errorbar(1, perm_mean, perm_ste, '.', 'LineStyle', 'none', 'Color', [0 1 1]) % cyan
        end % combI
        ...
        for combI=1:n_task_combinations   
            ...
            if ~strcmp(task_combinations{combI}{1}, task_combinations{combI}{2})
                ...
                subplot(nHors, nVers, combI);
                errorbar(1, noise_ceiling, 0, '.', 'LineStyle', 'none', 'Color', [0 0.4470 0.7410]); % blue
        ...
        pageHeading{1}          = sprintf('simmat corrmat replicability for time series'); 
...
close all; 
end % START_C6_topoInference_t_test
